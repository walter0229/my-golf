...
(function (g) {
  ...
  /* 스코어 이름 -> 파 대비 타수.
     음성인식 표기 흔들림(따블/트리쁠 등)도 함께 받는다. 긴 것부터 본다. */
  var SCORE_WORDS = [
    ['알바트로스', -3], ['앨버트로스', -3],
    ['더블보기', 2], ['따블보기', 2],
    ['트리플보기', 3], ['트리쁠보기', 3],
  ...
  function parse(text, hole, unit) {
    ...
    /* 3) 퍼팅
       음성인식이 "퍼트"를 매번 같은 글자로 적어 주지 않는다.
       실제로 투펏 / 투퍼트 / 투 펏 / 2펏 / 투풋 처럼 제각각 나오므로 넓게 받는다. */
    var PUTT = '(?:퍼팅|퍼트|퍼터|퍼티|펏|펕|펐|풋|붓|펀트|펌트)';
    var CNT = '(원|투|쓰리|스리|포|훠|한|하나|두|둘|세|셋|네|넷|다섯|일|이|삼|사|오|\\d)';
    var pm = take(new RegExp(CNT + '\\s*' + PUTT))
          || take(new RegExp(PUTT + '\\s*' + CNT));
    if (pm) out.putts = toNum(pm[1]);
    else if (new RegExp('노' + PUTT + '|' + PUTT + '없|' + PUTT + '안').test(flat)) {
      out.putts = 0;
      flat = flat.replace(new RegExp('노' + PUTT + '|' + PUTT + '없\\S*|' + PUTT + '안\\S*', 'g'), ' ');
    }
    ...
    /* 5) 파온 / 온그린  ("파"보다 먼저 소비해야 한다)
       쓰리온·포온은 규정 타수를 넘겨 올린 것이므로 파온 실패로 본다. */
    if (/쓰리온|스리온|삼온|포온|사온|네번만에온/.test(flat)) {
      out.gir = false; out.labels.push('파온 실패');
      flat = flat.replace(/쓰리온|스리온|삼온|포온|사온|네번만에온/g, ' ');
    } else if (/파온|온그린|그린온|그린올|투온|원온|온됐|올렸/.test(flat)) {
      out.gir = true; out.labels.push('파온 성공');
      flat = flat.replace(/파온|온그린|그린온|그린올|투온|원온|온됐\S*|올렸\S*/g, ' ');
    } else if (/노온|파온실패|온못|그린놓|미스그린/.test(flat)) {
      out.gir = false; out.labels.push('파온 실패');
      flat = flat.replace(/노온|파온실패|온못\S*|그린놓\S*|미스그린/g, ' ');
    }
